...
export default {
  data() {
    return {
      ...
      searchParams: {
        ...
        dateType: this.$route.query.dateType || 'day'
      },
      totalSize: 0,
      issuerList: [],
      channelList: [],
  ...
  methods: {
    ...
    getTableList(page) {
      ...
      cardServer.goodsDetail(this.searchParams).then(
        response => {
  ...
  async created() {},
  async mounted() {
    if (this.$route.query.dateType == 'year') {
      this.searchParams.settDate = this.$route.query.date + '-01-01'
    } else if (this.$route.query.dateType == 'month') {
      this.searchParams.settDate = this.$route.query.date + '-01'
    } else {
      this.searchParams.settDate = this.$route.query.date
    }
    this.getSelectData(1)
    await this.getSelectData(14, this.searchParams.issuerCode)
    this.getTableList()
  },
  updated() {}
}
