<template>
  <div>
    <el-dialog title="入金分类查看" :visible.sync="dialogTableVisible">
...
export default {
  ...
  methods: {
    createColumnOne(index) {
      ...
      switch (index) {
        case 0:
          str = this.detailData.chargeAmt
          break;
        case 1:
          str = this.detailData.cardRefundAmt
          break;
        case 2:
    ...
    createColumnTwo(index) {
      ...
      switch (index) {
        case 0:
          str = this.detailData.chargeCnt
          break;
        case 1:
          str = this.detailData.cardRefundCnt
          break;
        case 2:
    ...
    //获取入金分类查看
    getDepositDialog(){    
      const data = JSON.parse(sessionStorage.getItem('depositObj')) || {}
      console.log(data)
      cardServer.depositView(data).then(
        response => {
