<template>
    <div>
        <div class="title-box"
             v-show="true">{{billType=='day' ? this.currentMonth : this.currentMonth.slice(0,5)}} 商户交易账单
        </div>
        <div class="option-box">
            ...
            <div class="each div-inline " v-if="merchantType=='company'">
                <el-tag type="info"
                        plain>发卡主体
                </el-tag>
            ...
            <div class="each div-inline " v-else>
                <el-tag type="info"
                        plain>交易商户
                </el-tag>
                <el-select v-model="issuerCode"
                ...
                <div class="quarter">                        
                    <div class="eachItem" v-for="(item,index) in monthList" :key="index" v-if="index<3">
                        ...
                        <div class="monthBtn">
                            ...
                            v-if="item.status==3"
                            placement="top-start"
                            @show="onMouseHover(item.billTime,'month')"
                            trigger="hover">
                            <!-- <el-button slot="reference">hover 激活</el-button> -->
                                ...
                                <el-button slot="reference" style="width:125px;" type="primary" @click="onFileDownload(item.downUrl)">{{item.desc||'下载'}}</el-button>
                            </el-popover>
...
<script>
    ...
    export default {
        ...
        computed: {
            validParameter() {
                return (this.issuerCode&&this.merchantType=='company') || (this.issuerCode&&this.merchantType=='bus') 
            },
            currentMonth() {
        ...
        methods: {
            ...
            onMouseHover(date,type) {
                console.log(date)
                billServer.billSummary({
                    billType: type ? 'month' : 'day',
                    time: type ? moment(date).format("YYYYMM") : moment(date).format("YYYYMMDD"),
                    issuerCode: this.issuerCode
                }).then(
                    res => {
                        // console.log(res)
                        this.popoverOption = res
                    },
                    err => {
            ...
            getUserType() {
                billServer.getUserType().then(
                    response => {
                        this.merchantType = response.merchantType
                        if(response.merchantType) {
                            response.merchantType=='company' && this.getSelectData(1)
                            response.merchantType=='bus' && this.getSelectData(10)
                        }
                    },
                    response => {
                        this.$message.error(response.msg || '服务器连接失败！');
